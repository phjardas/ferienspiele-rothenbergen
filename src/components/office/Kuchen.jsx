...
export default function Kuchen() {
  ...
  useEffect(
    () =>
      getRegistrations({}, (error, registrations) =>
  ...
  const byDate = registrations.reduce(
    (acc, reg) => ({
      ...acc,
      [reg.kuchen.date]: [...(acc[reg.kuchen.date] || []), reg],
    }),
    {},
  );
  ...
  return (
    <Grid container spacing={2}>
      {config.kuchen.map((req) => (
        <Grid key={req.date} item size={{ xs: 12 }}>
          <KuchenStatistics
            date={req.date}
            required={req.amount}
            actual={byDate[req.date] || []}
          />
        </Grid>
      ))}
      {"none" in byDate && (
              ...
              {byDate.none.map((reg) => (
                <ListItem key={reg.id}>
                  <ListItemText
                    ...
                    secondary={
                      <>
                        <Link href={`tel:${reg.parent.phone}`}>
                          {reg.parent.phone}
                        </Link>
                        <br />
                        <Link href={`mailto:${reg.parent.email}`}>
                          {reg.parent.email}
                        </Link>
                      </>
                    }
