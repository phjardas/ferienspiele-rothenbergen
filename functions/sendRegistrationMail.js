...
function createWelcomeMail(reg, config) {
  ...
  let text = `Herzlichen Dank für die Anmeldung ${child.gender === 'w' ? 'Ihrer Tochter' : 'Ihres Sohnes'} ${child.firstName} ${child.lastName} zu den Ferienspielen Rothenbergen\n`;
  text += '\n';
  ...
  text += '\n';
  text += `3. Bitte drucken und unterschreiben Sie die Einverständniserklärung und geben Sie sie bis zum ${date(waiverDeadline)} bei uns ab.\n`;
  text += '\n';
