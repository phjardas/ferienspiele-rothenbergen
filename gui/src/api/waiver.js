...
export function createWaiver(reg) {
  const doc = new PDF({
  ...
  normal(
    `Hiermit erkläre ich mich einverstanden, dass ${reg.child.firstName} ${reg.child.lastName} \n` +
      `an den Kinderferienspielen Rothenbergen vom ${date(config.startDate)} bis ${date(config.endDate)} teilnehmen darf.`
  );
...
notwendigen Zahlungen geleistet sind.`
  ...
  normal(
    `Bei einer Absage durch teilnehmende Personen kann die Rückerstattung des Beitrages nach dem ${date(config.registrationDeadline)} nicht
garantiert werden.`
  );
